...
def mainPage() {
	dynamicPage(name: "mainPage", title: "${getVersionLabel()}", uninstall: true, install: true) {
        ...
        section(title: "") {
        	String defaultLabel = "Humidity Alert Multi!!"
        ...
        section("Monitor the humidity of:") {
            ...
            if (settings.humiditySensors?.size() > 1) {
                ...
                humidityNow = humiditySensors ? humiditySensors[0].currentHumidity.toString().replace("%", "") + '%' : '--'
            }
        ...
        section( "Notifications" ) {
            input "sendPushMessage", "enum", title: "Send a push notification?", options:["Yes","No"], required:true, defaultValue: "No"
...
def initialize() {
    ...
    if (settings.humiditySensors?.size() > 1) {
        if (settings.multiMode == 'average') {
            ...
            humidityNow = total / humiditySensors.size()
			humidityNowText = String.format('%.1f', humidityNow)+'%'
        } else {
            humiditySensors.each {
                humidityNowText = humidityNowText ? humidityNowText + ', ' + it.currentHumidity.toString().replace("%", "")+'%' : it.currentHumidity.toString().replace("%", "")+'%'
            }
        }
    } else {
        humidityNow = humiditySensors[0].currentHumidity
		humidityNowText = humidityNow.toString().replace("%", "") + '%'
    }
	if (humidityNowText) log.info "Current humidity is: ${humidityNowText}"
	if (theSwitch) {
		String switchStatus = theSwitch.currentSwitch
		if ((switchStatus == 'on') && (humidityNow < humidityLow)) {theSwitch.off(); switchStatus = 'off'; }
		log.info "${theSwitch.displayName} is currently ${switchStatus}"
    	updateMyLabel((switchStatus == 'on'))
...
def humidityHandler(evt) {
	...
	def currentHumidity = evt.value as BigDecimal
...
void updateMyLabel( on ) {
    ...
    String newLabel = myLabel + (on ? '<span style="color:red"> (running)</span>' : '<span style="color:green"> (idle)</span>')
